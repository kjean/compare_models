...
###### plot R0 and CVC
load(paste0(commondir, "YF_FOI_burden_model/", "p_prop3d.Rdata"))
load(paste0(commondir, "YF_FOI_burden_model/", "P_tot_2d.Rdata"))
load(paste0(commondir, "YF_FOI_burden_model/", "vc3d.Rdata"))
dim(vc3d)
...
image.plot(legend.only=T,breaks=mybreaks2,col=mycols2,zlim=c(0,1), 
           axis.args=list( at=log(log(ticks2)), labels=labticks2), legend.cex=3)
legend("bottomleft", fill = "#636363", legend="CVC reached 2014", cex = 1.5)
dev.off()
...
############################
#same with sup_cvc
R0_sup_20 = ifelse(R0_sup>20, 20,R0_sup)
R0_sup_10 = ifelse(R0_sup>10, 10,R0_sup)
log_R0_sup = log(R0_sup_20)
log_log_R0_sup = log(log(R0_sup_20))


cvc_sup = 1-1/R0_sup
range(cvc_sup)
cvc_reached_sup = rep(NA, dim(vc3d)[1])
cvc_reached_sup[vac_cov_2014>cvc_sup]=T
cvc_reached_sup[vac_cov_2014<=cvc_sup]=F
names(cvc_reached_sup) = dimnames(vc3d)[[1]]

mybreaks2= seq(min(log_log_R0_sup)-0.00001, log(log(10))+000001, length.out=101)
#mycols2 =  heat.colors(length(mybreaks2)-1)
mycols2 = rev(colorRampPalette(brewer.pal(11, "RdBu"))(100)) 
#mycols2 = rev(colorRampPalette(brewer.pal(11, "RdYlBu"))(100)) 
mm = match(shp1$adm0_adm1,dat$adm0_adm1)
vcols2 = findInterval(log_log_R0_sup,mybreaks2)
#vcols2=101-vcols2


png("map_R0_and_CVC_IC95sup.png", width=16,height=9,units="in",res=100)
par(mfrow = c(1,2))
#par(oma=c(2,4,4,4))
par(mar=c(2,4,4,6))
#png("map_log_R0.png", width=16,height=12,units="in",res=200)
...
plot(shp0[is.na(mm0),],col="grey80",add=T) 
plot(shp1[!is.na(mm),],col=mycols2[vcols2], xlim=c(-15,45),ylim=c(-18,30) , lty=0, add=T)#,border=mycols[vcols])
mm_cvc = match(names(cvc_reached_sup)[cvc_reached_sup==T], shp1$adm0_adm1)
plot(shp1[mm_cvc,], col="#636363", add=T)
plot(shp0, lwd=2, add=T)
#par( lab.cex=2.0)
image.plot(legend.only=T,breaks=mybreaks2,col=mycols2,zlim=c(0,1), 
           axis.args=list( at=log(log(ticks2)), labels=labticks2), legend.cex=3)
legend("bottomleft", fill = "#636363", legend="CVC reached 2014 (sup. bound IC95%)", cex = 1.5)
dev.off()
